...
public class DefaultHttpClientRequest implements HttpClientRequest {
  ...
  public HttpClientResponse execute(URI uri, String httpMethod, RequestHttpEntity requestHttpEntity)
      throws Exception {
    ClassicHttpRequest request = build(uri, httpMethod, requestHttpEntity);
    return new DefaultHttpClientResponse(client.execute(request));
  }

  private ClassicHttpRequest build(URI uri, String httpMethod, RequestHttpEntity requestHttpEntity)
      throws Exception {
    Object body = requestHttpEntity.getBody();
    BaseHttpMethod method = BaseHttpMethod.of(httpMethod, uri);
    Header headers = requestHttpEntity.getHeader();
    this.initRequestHeader(method, headers);
      ...
      if (form != null && !form.isEmpty()) {
        ...
        for (Map.Entry<String, Object> entry : form.entrySet()) {
          if (entry.getValue() instanceof File) {
            ...
            entityBuilder.addBinaryBody(
                entry.getKey(), inputStream, ContentType.DEFAULT_BINARY, "upload-file");
          } else {
  ...
  private void initRequestHeader(BaseHttpMethod request, Header headers) {
